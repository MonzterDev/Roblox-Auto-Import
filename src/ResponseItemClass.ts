...
export class ResponseItemClass {
    ...
    private CleanupImport ( document: ScriptDocument ) {
        const newLines = document.GetText().split( "\n" );
        const prefix = this.type === "Module" ? GetState().prefix.module : GetState().prefix.service
        const importStatement = document.GetText().split( "\n" ).find( ( line ) => line.find( prefix + this.instance.GetFullName() )[0] !== undefined && line.find( "require" )[0] === undefined && line.find( "game:GetService" )[0] === undefined )
        ...
        const text = importStatement.gsub( `${prefix}${this.instance.GetFullName()}`, this.label )[0]
    ...
    public Import ( document: ScriptDocument ) {
        ...
        task.defer( () => this.CleanupImport( document ) )
